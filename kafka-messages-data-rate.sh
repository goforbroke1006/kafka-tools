...
function get_kafka_total_bytes() {
  kafkaBrokers=$1
  kafkaTopic=$2
  timeFrom=$3
  timeTill=$4
  ...
  msgLengthArr=(
    $(
      kafkacat -C \
        -b "${kafkaBrokers}" -t "${kafkaTopic}" -o s@"${timeFrom}" -o e@"${timeTill}" \
        -q -e -f '%S '
    )
  )

  messagesCount=${#msgLengthArr[@]}
  if [[ $messagesCount -lt 2 ]]; then
    echo "Data rate: 0 bytes per second"
    exit 0
  fi

  bytesTotal=0
  for i in "${msgLengthArr[@]}"; do
    bytesTotal=$((bytesTotal + i))
  done
...
if [ $# -eq 1 ] && [ "$1" == '-h' ]; then
  echo "Script returns messages data volume rate (in bytes) per second."
...
bytesTotal=$(get_kafka_total_bytes "${kafkaBrokers}" "${kafkaTopic}")

messagesRatePerSec=$(bc <<<"scale=12; ${bytesTotal}/${timeRange}*1000")

echo "Data rate: ${messagesRatePerSec} bytes per second"
